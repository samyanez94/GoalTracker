...
@MainActor struct GoalFormModelTests {
	@Test func `Create mode starts with empty outcome goal defaults`() {
	...
	@Test func `Changing form data marks model as changed`() {
		let model = GoalFormModel(mode: .create)
	...
	@Test func `Reverting form data clears changed state`() {
		let model = GoalFormModel(mode: .create)
	...
	@Test func `Hidden progress values do not mark outcome goal as changed`() {
		let model = GoalFormModel(mode: .create)
	...
	@Test func `Enabling progress tracking marks model as changed`() {
		let model = GoalFormModel(mode: .create)
	...
	@Test func `Edit mode initializes from measurable form data`() {
		let dueDate = Date(timeIntervalSinceReferenceDate: 123)
		let tags = [Tag(name: "Health"), Tag(name: "Running")]
		let model = GoalFormModel(
			mode: .edit(
				GoalFormData(
					...
					dueDate: dueDate,
					progress: .measurable(currentValue: 3, targetValue: 10, step: 2, unit: .miles, ),
					tags: tags,
				),
			),
		)
	...
	@Test func `Edit mode initializes from recurring form data`() {
	...
	@Test func `Empty or whitespace name disables saving`() {
		let model = GoalFormModel(mode: .create)
	...
	@Test func `Invalid measurable values disable saving`() {
		let model = GoalFormModel(mode: .create)
	...
	@Test func `Default measurable values allow saving`() {
		let model = GoalFormModel(mode: .create)
		model.name = "Read books"
		model.isProgressBased = true
	...
	@Test func `Edit mode allows saving complete measurable progress`() {
		let model = GoalFormModel(
			mode: .edit(
				GoalFormData(
					name: "Read books",
	...
	@Test func `Form data includes due date only when due date is enabled`() {
	...
	@Test func `Selecting recurrence clears due date and preserves reminder`() {
		let dueDate = Date(timeIntervalSinceReferenceDate: 456)
		let reminder = GoalReminder()
		let model = GoalFormModel(mode: .create)
		model.name = "Run"
	...
	@Test func `Recurring edit data ignores existing due date and preserves reminder`() {
		let reminder = GoalReminder()
		let model = GoalFormModel(
			mode: .edit(
				GoalFormData(
	...
	@Test func `Enabling due date does not default reminder`() {
		let model = GoalFormModel(mode: .create)
	...
	@Test func `Enabling due date preserves existing reminder`() {
		let reminder = GoalReminder()
		let model = GoalFormModel(mode: .create)
		model.reminder = reminder
	...
	@Test func `Disabling due date clears reminder`() {
		let model = GoalFormModel(mode: .create)
		model.hasDueDate = true
		model.setDueDateEnabled(true)
	...
	@Test func `Edit mode preserves reminder in form data`() {
		let reminder = GoalReminder()
		let model = GoalFormModel(
			mode: .edit(
				GoalFormData(
	...
	@Test func `Form data trims name and preserves details for normalization`() {
		let model = GoalFormModel(mode: .create)
	...
	@Test func `Form data preserves selected tags`() {
		let tags = [Tag(name: "Health"), Tag(name: "Running")]
		let model = GoalFormModel(
			mode: .edit(
				GoalFormData(name: "Run", details: "", progress: .outcomePending, tags: tags, ),
			),
		)
	...
	@Test func `Form data preserves selected recurrence`() {
		let model = GoalFormModel(mode: .create)
		model.name = "Read"
		model.recurrence = GoalRecurrence(cadence: .daily)
	...
	@Test func `Form data preserves never recurrence`() {
		let model = GoalFormModel(
			mode: .edit(
				GoalFormData(
					name: "Read",
	...
	@Test func `Completed outcome edit remains completed in form data`() {
		let model = GoalFormModel(
			mode: .edit(GoalFormData(name: "Book trip", details: "", progress: .outcomeCompleted, ),
			),
		)
	...
	@Test func `Due date expansion follows due date enablement`() {
		let model = GoalFormModel(mode: .create)
