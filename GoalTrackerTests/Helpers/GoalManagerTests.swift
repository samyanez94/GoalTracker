...
@MainActor struct GoalManagerTests {
	@Test func `Incrementing measurable progress appends timestamped event`() async throws {
		let container = try makeContainer()
		let timestamp = Date(timeIntervalSinceReferenceDate: 123)
		let goal = makeGoal(progress: .measurable(currentValue: 4, targetValue: 10, step: 2), )
	...
	@Test func `Completing outcome goal appends timestamped event`() async throws {
		let container = try makeContainer()
		let timestamp = Date(timeIntervalSinceReferenceDate: 123)
	...
	@Test func `Recurring measurable progress updates current cadence period only`() async throws {
		let container = try makeContainer()
		...
		let goal = makeGoal(
			progress: GoalProgress(
				kind: .measurable,
				events: [GoalProgressEvent(delta: 10, timestamp: yesterday)],
				targetValue: 10,
				step: 5,
			),
	...
	@Test func `Custom progress update appends signed amount`() async throws {
		let container = try makeContainer()
		let timestamp = Date(timeIntervalSinceReferenceDate: 123)
		let goal = makeGoal(progress: .measurable(currentValue: 4, targetValue: 10, step: 2), )
		insert(goal, into: container)
	...
	@Test func `Recurring custom progress update applies to current cadence period only`()
		async throws
	{
		let container = try makeContainer()
		let yesterday = date(year: 2026, month: 5, day: 27, hour: 12)
	...
	@Test func `Updating recurrence preserves progress history`() async throws {
		let container = try makeContainer()
		...
		let goal = makeGoal(
			progress: GoalProgress(
				kind: .outcome,
				events: [GoalProgressEvent(delta: 1, timestamp: completionDate)],
				targetValue: 1,
				step: 1,
			),
		)
		...
		try manager.updateRecurrence(goal, recurrence: GoalRecurrence(cadence: .daily), )
	...
	@Test func `Updating goal with form data sets recurrence`() async throws {
		let container = try makeContainer()
		let goal = makeGoal(progress: .outcomePending)
		insert(goal, into: container)
	...
	@Test func `Updating goal with form data clears recurrence`() async throws {
		let container = try makeContainer()
		let goal = makeGoal(progress: .outcomePending, recurrence: GoalRecurrence(cadence: .daily),
		)
		insert(goal, into: container)
		let manager = makeManager(in: container)
	...
	@Test func `Direct goal update preserves recurrence`() async throws {
		let container = try makeContainer()
		let recurrence = GoalRecurrence(cadence: .weekly)
		let goal = makeGoal(progress: .outcomePending, recurrence: recurrence, )
		insert(goal, into: container)
		let manager = makeManager(in: container)
	...
	@Test func `Editing measurable current value appends balancing event`() async throws {
		let container = try makeContainer()
		let timestamp = Date(timeIntervalSinceReferenceDate: 123)
		let goal = makeGoal(progress: .measurable(currentValue: 4, targetValue: 10, step: 2), )
		insert(goal, into: container)
	...
	@Test func `Editing measurable target and step preserves existing events`() async throws {
		let container = try makeContainer()
	...
	@Test func `Editing recurring goal reminder preserves current period progress`() async throws {
		let container = try makeContainer()
		let yesterday = date(year: 2026, month: 5, day: 27, hour: 12)
		let today = date(year: 2026, month: 5, day: 28, hour: 12)
	...
	@Test func `Editing goal updates reminder`() async throws {
		let container = try makeContainer()
	...
	@Test func `Editing goal updates selected tags`() async throws {
		let container = try makeContainer()
		let goal = makeGoal(progress: .outcomePending)
	...
	@Test func `Editing goal deletes tags that are no longer used`() async throws {
		let container = try makeContainer()
	...
	@Test func `Deleting a goal removes it from the model context`() async throws {
		let container = try makeContainer()
		let goal = makeGoal(progress: .measurable(currentValue: 4, targetValue: 10, step: 2), )
		insert(goal, into: container)
		let manager = makeManager(in: container)
	...
	@Test func `Deleting a goal deletes tags that are no longer used`() async throws {
		let container = try makeContainer()
	...
	@Test func `Deleting a goal keeps tags used by another goal`() async throws {
		let container = try makeContainer()
	...
	@Test func `Deleting a tag removes it from goals`() throws {
		let container = try makeContainer()
	...
	@Test func `Deleting unused tags keeps protected form selections`() throws {
		let container = try makeContainer()
	...
	@Test func `Deleting multiple goals removes them from the model context`() async throws {
		let container = try makeContainer()
		...
		let retainedGoal = makeGoal(name: "Retained Goal", progress: .outcomeCompleted, )
	...
	@Test func `Bulk delete save failure rolls back deletions and throws`() async throws {
		let container = try makeContainer()
		let firstGoal = makeGoal(
			name: "First Goal",
			progress: .measurable(currentValue: 4, targetValue: 10, step: 2),
		...
		let manager = GoalManager(
			...
			saveContext: { throw TestSaveError.failed },
		)
	...
	@Test func `Update save failure rolls back goal changes and tag cleanup`() async throws {
		let container = try makeContainer()
		let oldTag = Tag(name: "Old")
		let newTag = Tag(name: "New")
		let goal = makeGoal(name: "Original Goal", progress: .outcomePending, )
		goal.tags = [oldTag]
		insert(goal, into: container)
		container.mainContext.insert(newTag)
		try container.mainContext.save()
	...
	@Test func `Save failure rolls back progress changes and throws`() async throws {
		let container = try makeContainer()
		let goal = makeGoal(progress: .measurable(currentValue: 4, targetValue: 10, step: 2), )
		insert(goal, into: container)
		let manager = GoalManager(
		...
		#expect(throws: GoalManager.SaveError.self) { try manager.incrementProgress(goal) }
		#expect(goal.progress.currentValue == 4)
	}

	@Test func `Adding a goal with a due date schedules notification reminder`() async throws {
		let container = try makeContainer()
		let scheduler = FakeGoalReminderScheduler()
		let goal = makeGoal(
			dueDate: Date(timeIntervalSinceReferenceDate: 60 * 60 * 24 * 30),
	...
	@Test func `Editing a goal without reminder eligibility delegates reminder sync`() async throws
	{
		let container = try makeContainer()
		let scheduler = FakeGoalReminderScheduler()
		let goal = makeGoal(
	...
	@Test func `Completing a goal delegates reminder sync`() async throws {
		let container = try makeContainer()
		let scheduler = FakeGoalReminderScheduler()
		let goal = makeGoal(
			dueDate: Date(timeIntervalSinceReferenceDate: 60 * 60 * 24 * 30),
	...
	@Test func `Restoring a completed goal delegates reminder sync`() async throws {
		let container = try makeContainer()
		let scheduler = FakeGoalReminderScheduler()
		let goal = makeGoal(
			dueDate: Date(timeIntervalSinceReferenceDate: 60 * 60 * 24 * 30),
	...
	@Test func `Deleting goals cancels notification reminders`() async throws {
		let container = try makeContainer()
		let scheduler = FakeGoalReminderScheduler()
	...
	private func insert(_ goal: Goal, into container: ModelContainer, ) {
	...
	private func fetchTags(in container: ModelContainer) throws -> [GoalTrackerSchemaV1.Tag] {
		try container.mainContext.fetch(
			FetchDescriptor<GoalTrackerSchemaV1.Tag>(sortBy: [
				SortDescriptor<GoalTrackerSchemaV1.Tag>(\.normalizedName)
			], ),
		)
	}
...
@MainActor private final class FakeGoalReminderScheduler: GoalReminderScheduling {
	var syncResult = true
	...
	func syncReminder(for state: GoalReminderSyncState, requestsAuthorization: Bool, ) async throws
		-> Bool
	{
	...
	func cancelReminders(for goalIds: [UUID]) { canceledGoalIds.append(contentsOf: goalIds) }
}
