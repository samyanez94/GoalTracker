...
@MainActor struct TagTests {
	@Test(arguments: [
		(" Health ", "Health", "health"), ("#Health", "Health", "health"),
		("#  Health  ", "Health", "health"), ("Home Gym", "HomeGym", "homegym"),
	...
	@Test func `Goal tag relationship persists in model container`() throws {
		...
		let tag = Tag(name: "#Health", createdAt: Date(timeIntervalSinceReferenceDate: 1), )
	...
	@Test func `Alphabetical sort descriptor sorts tags by normalized name`() throws {
		let container = try GoalTrackerModelContainer.make(isStoredInMemoryOnly: true)
		for tag in [Tag(name: "Zebra"), Tag(name: "apple"), Tag(name: "Banana")] {
			container.mainContext.insert(tag)
		}
		try container.mainContext.save()
