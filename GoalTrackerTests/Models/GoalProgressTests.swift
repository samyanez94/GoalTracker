...
@MainActor struct GoalProgressTests {
	@Test func `Measurable progress stores initial value as a timestamped event`() throws {
		let timestamp = Date(timeIntervalSinceReferenceDate: 123)

		let progress = makeProgress(currentValue: 3, targetValue: 10, timestamp: timestamp, )
	...
	@Test func `Zero progress stores no events`() {
	...
	@Test func `Complete appends delta to target`() throws {
	...
	@Test func `Reset appends negative delta to zero`() throws {
		var progress = makeProgress(currentValue: 3, targetValue: 10)
		let timestamp = Date(timeIntervalSinceReferenceDate: 456)
	...
	@Test func `Increment increases current value by step`() throws {
	...
	@Test func `Decrement decreases current value by step`() throws {
	...
	@Test func `Increment clamps at target value`() throws {
	...
	@Test func `Decrement clamps at zero`() throws {
	...
	@Test func `Custom update increases current value by amount`() throws {
	...
	@Test func `Custom update decreases current value by amount`() throws {
	...
	@Test func `Custom update clamps at target value`() throws {
		var progress = makeProgress(currentValue: 2, targetValue: 10)
	...
	@Test func `Custom update clamps at zero`() throws {
		var progress = makeProgress(currentValue: 2, targetValue: 10)
	...
	@Test func `Custom update ignores zero and non-finite amounts`() {
		var progress = makeProgress(currentValue: 2, targetValue: 10)
	...
	@Test func `Progress methods report whether state changed`() {
	...
	@Test func `isCompleted is true when current value reaches target`() {
	...
	@Test func `Period progress ignores events outside period`() {
	...
	@Test func `Period completion uses only current period target progress`() {
		let period = DateInterval(
			start: Date(timeIntervalSinceReferenceDate: 100),
			end: Date(timeIntervalSinceReferenceDate: 200),
		)
	...
	@Test func `Completing period appends event without clearing prior history`() throws {
		let period = DateInterval(
			start: Date(timeIntervalSinceReferenceDate: 100),
			end: Date(timeIntervalSinceReferenceDate: 200),
		)
	...
	@Test func `Period increment and decrement mutate current period value`() {
		let period = DateInterval(
			start: Date(timeIntervalSinceReferenceDate: 100),
			end: Date(timeIntervalSinceReferenceDate: 200),
		)
	...
	@Test func `Period custom update mutates current period value`() throws {
		let period = DateInterval(
			start: Date(timeIntervalSinceReferenceDate: 100),
			end: Date(timeIntervalSinceReferenceDate: 200),
		)
	...
	@Test func `fractionCompleted returns current value divided by target`() {
		let progress = makeProgress(currentValue: 4, targetValue: 10)

		#expect(progress.fractionCompleted == 0.4)
	}

	@Test func `Outcome progress acts like zero or one progress`() {
	...
	@Test func `Outcome progress does not step incrementally`() {
	...
	@Test func `Updated progress preserves events when current value is unchanged`() {
	...
	@Test func `Updated progress appends balancing event when current value changes`() {
	...
	@Test func `Valid progress values decode successfully`() throws {
	...
	@Test func `Invalid negative progress values throw a data corrupted error`() {
	...
	@Test func `Historical progress may exceed one target value`() throws {
		let progress = try decodeProgress(
			"""
			{
			    "kind": "measurable",
	...
	@Test func `Missing step decodes with default of one`() throws {
		let progress = try decodeProgress(
			"""
			{
			    "kind": "measurable",
	...
	@Test func `Legacy current value payload does not decode`() {
		#expect(throws: DecodingError.self) {
			try decodeProgress(
				"""
				{
