...
@MainActor struct GoalManager {
	...
	func updateGoal(
		...
		try saveChanges(
			performing: {
				...
				if updatesRecurrence { goal.recurrence = recurrence }
				if let tags {
					goal.tags = tags
					try deleteUnusedTags(from: previousTags + tags, excluding: tags, )
				}
			},
			restoreOnFailure: { snapshot.restore(goal) },
		)
		syncReminder(for: goal, requestsAuthorization: true)
	}

	/// Updates a goal using values collected by the goal form.
	func updateGoal(_ goal: Goal, with data: GoalFormData, ) throws {
		try updateGoal(
			goal,
			name: data.name,
	...
	func updateRecurrence(_ goal: Goal, recurrence: GoalRecurrence?, ) throws {
		let snapshot = GoalSnapshot(goal: goal)
		try saveChanges(
			performing: { goal.recurrence = recurrence },
			restoreOnFailure: { snapshot.restore(goal) },
		)
	...
	@discardableResult func toggleCompletion(_ goal: Goal, ) throws -> Bool {
		try updateProgress(goal) { goal in goal.toggleCompletion(timestamp: now()) }
	}

	/// Marks a goal as complete and saves the change.
	///
	/// - Returns: `true` when the goal's progress changed.
	@discardableResult func completeGoal(_ goal: Goal, ) throws -> Bool {
		try updateProgress(goal) { goal in goal.complete(timestamp: now()) }
	}

	/// Advances a measurable goal by its configured step and saves the change.
	///
	/// - Returns: `true` when the goal's progress changed.
	@discardableResult func incrementProgress(_ goal: Goal, ) throws -> Bool {
		try updateProgress(goal) { goal in goal.incrementProgress(timestamp: now()) }
	}

	/// Reduces a measurable goal by its configured step and saves the change.
	///
	/// - Returns: `true` when the goal's progress changed.
	@discardableResult func decrementProgress(_ goal: Goal, ) throws -> Bool {
		try updateProgress(goal) { goal in goal.decrementProgress(timestamp: now()) }
	}

	/// Applies a custom signed amount to a measurable goal's progress and saves the change.
	///
	/// - Returns: `true` when the goal's progress changed.
	@discardableResult func updateProgress(_ goal: Goal, by amount: Double, ) throws -> Bool {
		try updateProgress(goal) { goal in goal.updateProgress(by: amount, timestamp: now()) }
	}

	/// Deletes a single goal and removes any of its tags that are no longer used.
	func deleteGoal(_ goal: Goal) throws { try deleteGoals([goal]) }
	...
	func deleteGoals(_ goals: [Goal]) throws {
		...
		try saveChanges {
			try deleteUnusedTags(from: candidateTags, ignoringGoalsWithIds: deletedGoalIds, )
		}
	...
	func deleteUnusedTags(excluding protectedTags: [Tag] = []) throws {
		try saveChanges { try deleteUnusedTags(from: fetchTags(), excluding: protectedTags, ) }
	}
	...
	@discardableResult private func updateProgress(_ goal: Goal, _ mutate: (Goal) -> Bool, ) throws
		-> Bool
	{
		let snapshot = GoalSnapshot(goal: goal)
		guard mutate(goal) else { return false }
		try saveChanges(restoreOnFailure: { snapshot.restore(goal) })
		syncReminder(for: goal)
		return true
	}

	private func syncReminder(for goal: Goal, requestsAuthorization: Bool = false, ) {
	...
	private func deleteUnusedTags(
		...
		for tag in candidateTags where !protectedTagIds.contains(tag.id) {
			guard !checkedTagIds.contains(tag.id) else { continue }
			checkedTagIds.insert(tag.id)
			guard !usedTagIds.contains(tag.id) else { continue }
			modelContext.delete(tag)
		}
	}

	private func usedTagIds(ignoringGoalsWithIds ignoredGoalIds: Set<UUID>, ) throws -> Set<UUID> {
		let goals = try fetchGoals()
		return Set(
			goals.lazy.filter { goal in !ignoredGoalIds.contains(goal.id) }.flatMap(\.tags)
				.map(\.id)
		)
	}

	private func fetchGoals() throws -> [Goal] { try modelContext.fetch(FetchDescriptor<Goal>()) }
	...
	enum SaveError: LocalizedError {
		...
		var errorDescription: String? { "Your changes could not be saved." }
	}
}
