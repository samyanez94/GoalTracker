...
@MainActor protocol GoalReminderScheduling {
...
@MainActor struct GoalNotificationScheduler: GoalReminderScheduling {
	...
	@discardableResult func syncReminder(
		...
		let currentDate = now()
		guard let schedule = reminderSchedule(state: state, currentDate: currentDate, ) else {
			return false
		}

		if requestsAuthorization {
			guard try await requestAuthorizationIfNeeded() else { return false }
		}
	...
	/// - Returns: `true` when a notification request was scheduled.
	@discardableResult func syncReminder(for goal: Goal, requestsAuthorization: Bool) async throws
		-> Bool
	{
	...
	/// - Returns: `true` when a notification request was scheduled.
	@discardableResult func scheduleReminder(for goal: Goal) async throws -> Bool {
		try await syncReminder(for: goal, requestsAuthorization: false)
	}

	private func reminderSchedule(state: GoalReminderSyncState, currentDate: Date, )
		-> GoalReminderSchedule?
	{ GoalReminderSchedule.reminder(state: state, calendar: calendar, currentDate: currentDate, ) }

	private func scheduleReminder(_ schedule: GoalReminderSchedule, currentDate: Date, )
		async throws
	{
	...
	private func notificationBody(for schedule: GoalReminderSchedule, relativeTo currentDate: Date, )
		-> String
	{
		let dueDescription =
			switch schedule.dueDescription {
			case .date(let dueDate):
				relativeDueDateDescription(for: dueDate, relativeTo: currentDate, ).lowercased()
			case .cadence(let cadence): cadence.reminderDueDescription
			}
		return "Don't forget to complete by \(dueDescription)"
	}

	private func relativeDueDateDescription(for dueDate: Date, relativeTo currentDate: Date, )
		-> String
	{
		let currentDay = calendar.startOfDay(for: currentDate)
		let dueDay = calendar.startOfDay(for: dueDate)
		guard !calendar.isDate(dueDay, inSameDayAs: currentDay) else { return "Today" }
		if isExactOffset(.day, value: 1, from: currentDay, to: dueDay) { return "Tomorrow" }
		if isExactOffset(.weekOfYear, value: 1, from: currentDay, to: dueDay) { return "Next week" }
		if isExactOffset(.month, value: 1, from: currentDay, to: dueDay) { return "Next month" }
		if isExactOffset(.year, value: 1, from: currentDay, to: dueDay) { return "Next year" }
	...
	private func isExactOffset(
		...
		from startDate: Date,
		to endDate: Date,
	) -> Bool { calendar.date(byAdding: component, value: value, to: startDate) == endDate }
