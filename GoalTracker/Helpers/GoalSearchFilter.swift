...
struct GoalSearchFilter {
	func filtered(_ goals: [Goal], searchText: String, ) -> [Goal] {
		let trimmedSearchText = searchText.trimmingCharacters(in: .whitespacesAndNewlines)
		guard !trimmedSearchText.isEmpty else { return goals }
	...
	private func tagMatchesSearchText(_ tag: Tag, searchText: String, ) -> Bool {
		let tagSearchText =
			if searchText.starts(with: "#") { Tag.sanitizedName(from: searchText) } else {
				searchText
			}
