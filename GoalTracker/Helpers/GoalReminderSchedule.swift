...
struct GoalReminderSchedule {
	...
	static func reminder(state: GoalReminderSyncState, calendar: Calendar, currentDate: Date, )
		-> GoalReminderSchedule?
	{
		guard let reminder = state.reminder else { return nil }
		if let recurrence = state.recurrence {
			return recurringReminder(state: state, recurrence: recurrence, calendar: calendar, )
		}
	...
	private static func oneTimeReminder(
		...
		guard !state.progress.isCompleted, let dueDate = state.dueDate,
			let fireDate = reminder.fireDate(on: dueDate, calendar: calendar, ),
