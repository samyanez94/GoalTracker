...
nonisolated struct GoalProgress: Codable {
	...
	static let outcomeCompleted = GoalProgress(
		kind: .outcome,
		events: [GoalProgressEvent(delta: 1, timestamp: Date(timeIntervalSinceReferenceDate: 0))],
		targetValue: 1,
		step: 1,
		unit: nil,
	)
	...
	var currentValue: Double { boundedValue(from: eventValue) }

	/// Whether the current value has reached or exceeded the target value.
	var isCompleted: Bool { currentValue >= targetValue }

	var isMeasurable: Bool { kind == .measurable }

	var canDecrement: Bool { isMeasurable && currentValue > 0 }

	var canIncrement: Bool { isMeasurable && currentValue < upperBound }
	...
	init(
		...
		precondition(
			Self.isValid(events: events, targetValue: targetValue, step: step, ),
	...
	static func isValid(events: [GoalProgressEvent], targetValue: Double, step: Double, ) -> Bool {
		let currentValue = events.reduce(0) { $0 + $1.delta }
		return events.allSatisfy { $0.delta.isFinite } && currentValue.isFinite
			&& targetValue.isFinite && step.isFinite && currentValue >= 0 && targetValue > 0
			&& step > 0
	}

	static func isValid(currentValue: Double, targetValue: Double, step: Double, ) -> Bool {
		currentValue.isFinite && targetValue.isFinite && step.isFinite && currentValue >= 0
			&& targetValue > 0 && step > 0 && currentValue <= targetValue
	}

	@discardableResult mutating func complete(timestamp: Date = Date()) -> Bool {
		setCurrentValue(targetValue, timestamp: timestamp)
	}

	@discardableResult mutating func reset(timestamp: Date = Date()) -> Bool {
		setCurrentValue(0, timestamp: timestamp)
	}

	@discardableResult mutating func toggleCompletion(timestamp: Date = Date()) -> Bool {
		isCompleted ? reset(timestamp: timestamp) : complete(timestamp: timestamp)
	}

	@discardableResult mutating func increment(timestamp: Date = Date()) -> Bool {
		guard isMeasurable else { return false }
		return setCurrentValue(currentValue + step, timestamp: timestamp)
	}

	@discardableResult mutating func decrement(timestamp: Date = Date()) -> Bool {
		guard isMeasurable else { return false }
		return setCurrentValue(currentValue - step, timestamp: timestamp)
	}

	@discardableResult mutating func update(by amount: Double, timestamp: Date = Date(), ) -> Bool {
		guard isMeasurable, amount.isFinite else { return false }
		return setCurrentValue(currentValue + amount, timestamp: timestamp)
	}

	@discardableResult mutating func replaceCurrentValue(
		_ currentValue: Double,
		timestamp: Date = Date(),
	) -> Bool { setCurrentValue(currentValue, timestamp: timestamp) }

	func currentValue(in period: DateInterval) -> Double {
		boundedValue(from: eventValue(in: period))
	}

	func isCompleted(in period: DateInterval) -> Bool { currentValue(in: period) >= targetValue }
	...
	func fractionCompleted(in period: DateInterval) -> Double {
		guard targetValue > 0 else { return isCompleted(in: period) ? 1 : 0 }
		return currentValue(in: period) / targetValue
	}

	@discardableResult mutating func complete(in period: DateInterval, timestamp: Date = Date(), )
		-> Bool
	{ setCurrentValue(targetValue, in: period, timestamp: timestamp) }

	@discardableResult mutating func reset(in period: DateInterval, timestamp: Date = Date(), )
		-> Bool
	{ setCurrentValue(0, in: period, timestamp: timestamp) }

	@discardableResult mutating func toggleCompletion(
		in period: DateInterval,
		timestamp: Date = Date(),
	) -> Bool {
		isCompleted(in: period)
			? reset(in: period, timestamp: timestamp) : complete(in: period, timestamp: timestamp)
	}

	@discardableResult mutating func increment(in period: DateInterval, timestamp: Date = Date(), )
		-> Bool
	{
		guard isMeasurable else { return false }
		return setCurrentValue(currentValue(in: period) + step, in: period, timestamp: timestamp, )
	}

	@discardableResult mutating func decrement(in period: DateInterval, timestamp: Date = Date(), )
		-> Bool
	{
		guard isMeasurable else { return false }
		return setCurrentValue(currentValue(in: period) - step, in: period, timestamp: timestamp, )
	...
	@discardableResult mutating func update(
		...
		guard isMeasurable, amount.isFinite else { return false }
		return setCurrentValue(currentValue(in: period) + amount, in: period, timestamp: timestamp,
		)
	}

	func updated(preservingEventsFrom previousProgress: GoalProgress, timestamp: Date = Date(), )
		-> GoalProgress
	{
		guard kind == previousProgress.kind else { return self }
		let updatedCurrentValue = currentValue
		var updatedProgress = self
		updatedProgress.events = previousProgress.events
		guard updatedCurrentValue != previousProgress.currentValue else { return updatedProgress }
	...
	private var upperBound: Double { max(0, targetValue) }

	private var eventValue: Double { events.reduce(0) { $0 + $1.delta } }

	private func eventValue(in period: DateInterval) -> Double {
		events.reduce(0) { value, event in
			guard event.timestamp.isInsideProgressPeriod(period) else { return value }
			return value + event.delta
		}
	}

	private func boundedValue(from value: Double) -> Double { min(max(0, value), upperBound) }

	private static func events(for currentValue: Double, timestamp: Date, ) -> [GoalProgressEvent] {
		guard currentValue > 0 else { return [] }
		return [GoalProgressEvent(delta: currentValue, timestamp: timestamp)]
	}

	@discardableResult private mutating func setCurrentValue(_ value: Double, timestamp: Date, )
		-> Bool
	{
	...
	@discardableResult private mutating func setCurrentValue(
		_ value: Double,
		in period: DateInterval,
		timestamp: Date,
	) -> Bool {
...
nonisolated private struct GoalProgressUnitSnapshot: Codable {
	...
	func resolvedUnit() -> GoalProgressUnit? {
		...
		if let preset = GoalProgressUnit.preset(withID: id) { return preset }
