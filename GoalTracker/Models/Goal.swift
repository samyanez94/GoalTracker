...
extension GoalTrackerSchemaV1 {
	...
	@Model final class Goal {
		...
		var status: GoalStatus { status() }

		var isMeasurable: Bool { progress.isMeasurable }
		...
		func status(at date: Date = Date(), calendar: Calendar = .current, ) -> GoalStatus {
			if isCompleted(at: date, calendar: calendar) { return .completed }
			if currentProgressValue(at: date, calendar: calendar) > 0 { return .inProgress }
			return .pending
		}

		func currentProgressValue(at date: Date = Date(), calendar: Calendar = .current, ) -> Double
		{
			guard let period = recurrence?.period(containing: date, calendar: calendar) else {
		...
		func currentStreak(at date: Date = Date(), calendar: Calendar = .current, ) -> Int? {
			...
			if let currentPeriod = period, !progress.isCompleted(in: currentPeriod) {
				period = recurrence.period(before: currentPeriod, calendar: calendar)
			}
			while let currentPeriod = period, progress.isCompleted(in: currentPeriod) {
				streak += 1
				period = recurrence.period(before: currentPeriod, calendar: calendar)
			}
			return streak
		...
		func canDecrementProgress(at date: Date = Date(), calendar: Calendar = .current, ) -> Bool {
			guard let period = recurrence?.period(containing: date, calendar: calendar) else {
		...
		func canIncrementProgress(at date: Date = Date(), calendar: Calendar = .current, ) -> Bool {
			guard let period = recurrence?.period(containing: date, calendar: calendar) else {
		...
		@discardableResult func complete(timestamp: Date = Date(), calendar: Calendar = .current, )
			-> Bool
		{
		...
		@discardableResult func toggleCompletion(
			timestamp: Date = Date(),
		...
		@discardableResult func incrementProgress(
			timestamp: Date = Date(),
			calendar: Calendar = .current,
		) -> Bool {
			guard let period = recurrence?.period(containing: timestamp, calendar: calendar) else {
		...
		@discardableResult func decrementProgress(
			timestamp: Date = Date(),
			calendar: Calendar = .current,
		) -> Bool {
			guard let period = recurrence?.period(containing: timestamp, calendar: calendar) else {
		...
		@discardableResult func updateProgress(
			by amount: Double,
			timestamp: Date = Date(),
			calendar: Calendar = .current,
		) -> Bool {
