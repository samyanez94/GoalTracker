...
struct GoalRowView: View {
	...
	var body: some View {
		NavigationLink(value: goal) {
			HStack(spacing: 12) {
				Image(systemName: goal.status.iconSystemName).font(.title2)
				...
				VStack(alignment: .leading, spacing: 2) {
					Text(goal.name).foregroundStyle(goal.isCompleted ? .secondary : .primary)
					if let dueDate = goal.dueDate {
						Text(GoalDueDateFormatter.string(from: dueDate)).font(.subheadline)
							.foregroundStyle(isPastDue(dueDate) ? .red : .secondary)
					}
					if let recurrence = goal.recurrence {
						Text(recurrence.rowTitle).font(.subheadline).foregroundStyle(.secondary)
					}
		...
		.contextMenu {
			GoalActionMenuContent(
				...
				edit: { isPresentingEditForm = true },
		...
		.sheet(isPresented: $isPresentingEditForm) {
			NavigationStack {
				GoalFormView(mode: .edit(GoalFormData(goal: goal)), ) { data in
	...
	private func toggleCompletion() {
		do { try goalManager.toggleCompletion(goal) } catch { saveFailure = .updateProgress }
	}

	private func deleteGoal() {
		do { try goalManager.deleteGoal(goal) } catch { saveFailure = .deleteGoal }
	}
...
#Preview {
	...
	NavigationStack { List { ForEach(goals) { goal in GoalRowView(goal: goal, ) } } }
}
