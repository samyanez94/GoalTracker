...
struct GoalListView: View {
	...
	var body: some View {
		NavigationStack {
			Group {
				if goals.isEmpty {
					Text("No goals").font(.body).foregroundStyle(.secondary)
						.frame(maxWidth: .infinity, maxHeight: .infinity)
				} else if isSearching, visibleSearchResultsAreEmpty {
					Text("No matching goals").font(.body).foregroundStyle(.secondary)
						.frame(maxWidth: .infinity, maxHeight: .infinity)
				} else {
					List(selection: $selectedGoalIds) {
						if isShowingCompletedGoals {
							...
							ForEach(pendingGoals) { goal in GoalRowView(goal: goal).tag(goal.id) }
						}
					}
				}
			}
			.navigationTitle("Goals").environment(\.editMode, $editMode)
			...
			.toolbar {
				GoalListBottomToolbar(
					...
					onAddGoal: { isPresentingGoalFormView = true },
				...
				GoalListTopToolbar(
					...
					onSelectGoals: { editMode = .active },
					onFinishSelectingGoals: { finishSelectingGoals() }
				)
			}
			...
			.navigationDestination(for: Goal.self) { goal in GoalDetailView(goal: goal) }
			.goalSaveFailureAlert(failure: $saveFailure)
			.onChange(of: visibleGoalIds) { _, _ in pruneSelectedGoalIds() }
		}
	}
	...
	private var isSelectingGoals: Bool { editMode.isEditing }

	private var searchedGoals: [Goal] { searchFilter.filtered(goals, searchText: searchText, ) }
	...
	private var completedGoals: [Goal] {
		sorter.sorted(searchedGoals.filter(\.isCompleted), by: sortMode, direction: sortDirection, )
	}
	...
	private var visibleGoalIds: Set<UUID> { Set(visibleSelectableGoals.map(\.id)) }

	private var selectedGoals: [Goal] {
		visibleSelectableGoals.filter { goal in selectedGoalIds.contains(goal.id) }
	}
	...
	private func deleteSelectedGoals() {
		...
		guard !goalsToDelete.isEmpty else { return }
		do {
			try goalManager.deleteGoals(goalsToDelete)
			finishSelectingGoals()
		} catch { saveFailure = .deleteGoal }
	}

	private func pruneSelectedGoalIds() { selectedGoalIds.formIntersection(visibleGoalIds) }
}
