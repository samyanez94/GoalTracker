...
private enum ProgressUnitSelectionDestination: Hashable { case customUnit }
...
struct ProgressUnitSelectionView: View {
	...
	var body: some View {
		List {
			Section("None") {
				ProgressUnitButton(title: "None", unit: nil, selectedUnit: $selectedUnit, ) {
					dismiss()
				}
			}
			...
			Section {
				NavigationLink(value: ProgressUnitSelectionDestination.customUnit) {
					HStack {
						...
						Spacer()
						if let customUnit { Text(customUnit.title).foregroundStyle(.secondary) }
					}
				}
			}
		}
		.navigationTitle("Unit").navigationBarTitleDisplayMode(.inline)
