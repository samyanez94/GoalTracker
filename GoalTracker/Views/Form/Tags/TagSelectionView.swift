...
struct TagSelectionView: View {
	...
	init(selectedTags: Binding<[Tag]>) { _selectedTags = selectedTags }
	...
	var body: some View {
		Form {
			...
			Section {
				TextField("Add New Tag...", text: $newTagName).focused($newTagFieldIsFocused)
					.submitLabel(.done).textInputAutocapitalization(.words)
					.onChange(of: newTagName) { _, updatedName in sanitizeNewTagName(updatedName) }
	...
	private var hasAvailableTags: Bool { !availableTags.isEmpty }

	private var canShowEditButton: Bool { hasAvailableTags || isEditing }

	private func isSelected(_ tag: Tag) -> Bool {
		selectedTags.contains { selectedTag in selectedTag.normalizedName == tag.normalizedName }
	}
	...
	private func addTag() {
		...
		guard !sanitizedTagName.isEmpty else { return }
		let tag =
	...
	private func toggleEditMode() {
		isEditing.toggle()
		if isEditing { newTagFieldIsFocused = false }
	}
	...
	private func handleAvailableTagsEmptyChanged(_ tagsAreEmpty: Bool) {
		if tagsAreEmpty { isEditing = false }
	}

	private func deleteTag(_ tag: Tag) {
		let tagWasSelected = isSelected(tag)
		removeSelection(of: tag)
		do { try GoalManager(modelContext: modelContext).deleteTag(tag) } catch {
			if tagWasSelected { select(tag) }
	...
	private func removeSelection(of tag: Tag) {
		selectedTags.removeAll { selectedTag in selectedTag.normalizedName == tag.normalizedName }
	}
...
#Preview {
	NavigationStack { TagSelectionView(selectedTags: .constant([])) }
