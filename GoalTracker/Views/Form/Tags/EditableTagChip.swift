...
struct EditableTagChip: View {
	...
	var body: some View {
		ZStack(alignment: .topLeading) {
			TagChip(name: tag.name, isSelected: isSelected, isEnabled: !isEditing, ) {
				toggleSelection(tag)
			}
			.padding([.top, .leading], isEditing ? 6 : 0)
			if isEditing {
				Button("Delete Tag", systemImage: "xmark.circle.fill") {
					withAnimation { deleteTag(tag) }
				}
				.labelStyle(.iconOnly).buttonStyle(.plain)
				.foregroundStyle(.white, Color(.systemGray)).symbolRenderingMode(.palette)
			}
		}
	}
}
