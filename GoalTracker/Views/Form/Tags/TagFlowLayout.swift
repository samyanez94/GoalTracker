...
struct TagFlowLayout: Layout {
	...
	{
		let rows = makeRows(for: subviews, availableWidth: proposal.width ?? 0, )
		return CGSize(
			width: proposal.width ?? rows.map(\.width).max() ?? 0,
			height: rows.reduce(0) { height, row in height + row.height } + CGFloat(
				max(rows.count - 1, 0)
			) * spacing,
		)
	}
	...
	func placeSubviews(
		...
		for row in makeRows(for: subviews, availableWidth: bounds.width) {
			...
			for item in row.items {
				subviews[item.index]
					.place(at: CGPoint(x: x, y: y), proposal: ProposedViewSize(item.size), )
	...
	private func makeRows(for subviews: Subviews, availableWidth: CGFloat, ) -> [Row] {
		let items = subviews.indices.map { index in
			MeasuredItem(index: index, size: subviews[index].sizeThatFits(.unspecified), )
		}
		...
		guard availableWidth > 0 else {
			return [
				Row(
					items: items,
					width: items.reduce(0) { totalWidth, item in totalWidth + item.size.width },
					height: items.map { item in item.size.height }.max() ?? 0,
				)
			]
		}
		...
		appendRow(items: currentItems, width: currentWidth, height: currentHeight, to: &rows, )
