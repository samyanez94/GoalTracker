...
struct CustomUnitFormView: View {
	...
	init(initialUnit: GoalProgressUnit?, onSave: @escaping (GoalProgressUnit) -> Void, ) {
	...
	private var isSaveDisabled: Bool { trimmedName.isEmpty }
	...
	var body: some View {
		Form {
			Section {
				...
				Text("Values will display this text after the number.").font(.footnote)
					.foregroundStyle(.secondary)
			}
		}
		.navigationTitle("Custom Unit").navigationBarTitleDisplayMode(.inline)
		.toolbar {
			ToolbarItem(placement: .confirmationAction) {
				Button("Save", systemImage: "checkmark", action: save).labelStyle(.iconOnly)
					.tint(.blue).buttonStyle(.glassProminent).disabled(isSaveDisabled)
