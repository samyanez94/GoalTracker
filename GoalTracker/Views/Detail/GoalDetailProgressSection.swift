...
struct GoalDetailProgressSection: View {
	...
	var body: some View {
		VStack(alignment: .leading, spacing: 8) {
			Text("Progress").font(.headline).foregroundStyle(.secondary)
			HStack {
				VStack(alignment: .leading, spacing: 4) {
					Text(progressTitle).font(.title2.bold()).foregroundStyle(.primary)
						.contentTransition(.numericText(value: fractionCompleted))
					Text(progressSubtitle).font(.body).foregroundStyle(.secondary)
				}
				Spacer(minLength: 8)
				GoalDetailCircularProgressView(progress: fractionCompleted)
					.frame(width: 96, height: 96)
			}
			.padding(.all, 16).frame(maxWidth: .infinity, alignment: .leading)
			.background(
	...
	private var progress: GoalProgress { goal.progress }
	...
	private var progressTitle: String {
		let currentValue = formattedNumber(currentProgressValue, for: progress)
	...
	private var progressSubtitle: String {
		...
		let remainingText = formattedNumber(remainingValue, for: progress)
		return "\(remainingText) more to go"
	}

	private func formattedNumber(_ value: Double, for progress: GoalProgress) -> String {
		guard let prefix = progress.unit?.prefix else { return formattedNumber(value) }
	...
	private var unitText: String {
		guard let unit = progress.unit else { return "" }
		return unit.suffix ?? (unit.prefix == nil ? unit.abbreviatedTitle : "")
	}

	private var currentPeriod: DateInterval? { goal.recurrence?.period(containing: Date()) }
	...
	GoalDetailProgressSection(
		goal: Goal(
			...
			progress: .measurable(
				currentValue: 8,
				targetValue: 10,
				unit: .custom(title: "Marathons", abbreviatedTitle: "marathons", ),
			),
		)
	)
...
#Preview("Completed") {
	GoalDetailProgressSection(
		goal: Goal(
			...
			createdAt: Date(),
			progress: .measurable(currentValue: 2_500, targetValue: 2_500, unit: .dollars, ),
		)
	)
	.padding().background(Color(.systemGroupedBackground))
}
...
#Preview("Decimal Progress") {
	GoalDetailProgressSection(
		goal: Goal(
			...
			createdAt: Date(),
			progress: .measurable(currentValue: 2.5, targetValue: 5, unit: .kilometers, ),
		)
	)
	.padding().background(Color(.systemGroupedBackground))
}
