...
struct GoalDetailView: View {
	...
	var body: some View {
		ScrollView {
			VStack(alignment: .leading, spacing: 24) {
				...
				if goal.isRecurring { GoalDetailStreakSection(goal: goal) }
				switch goal.progress.kind {
				case .outcome: GoalDetailStatusSection(goal: goal)
				case .measurable: GoalDetailProgressSection(goal: goal)
				}
			}
		}
		.safeAreaPadding(.horizontal).background(Color(.systemGroupedBackground).ignoresSafeArea())
		.toolbar {
		...
		.sheet(isPresented: $isPresentingProgressUpdateSheet) {
			NavigationStack { GoalProgressUpdateView(goal: goal) }
		}
		.safeAreaInset(edge: .bottom) {
			GoalDetailBottomView(
				goal: goal,
				openProgressUpdateView: { isPresentingProgressUpdateSheet = true },
			)
		}
	...
	private func toggleCompletion() {
		do { guard try goalManager.toggleCompletion(goal) else { return } } catch {
