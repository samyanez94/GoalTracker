...
struct GoalDetailStreakSection: View {
	...
	var body: some View {
		if let streakDetails {
			VStack(alignment: .leading, spacing: 8) {
				Text("Streak").font(.headline).foregroundStyle(.secondary)
				HStack {
					Image(systemName: streakDetails.iconSystemName).font(.title2)
						.foregroundStyle(streakDetails.iconForegroundStyle)
						.contentTransition(.symbolEffect(.replace)).accessibilityHidden(true)
					Text("Current Streak")
					Spacer(minLength: 8)
					Text(streakDetails.title).foregroundStyle(.secondary)
	...
	private var streakDetails: StreakDetails? {
		guard let recurrence = goal.recurrence, let streak = goal.currentStreak() else {
			return nil
		}
