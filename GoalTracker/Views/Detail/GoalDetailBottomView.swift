...
struct GoalDetailBottomView: View {
	...
	var body: some View {
		Group {
			...
			case .outcome: CompleteGoalButton(isCompleted: goal.isCompleted, action: completeGoal)
			case .measurable:
				HStack(spacing: 6) {
	...
	private func completeGoal() {
		do {
			guard try goalManager.completeGoal(goal) else { return }
	...
	private func decrementProgress() {
		do {
			guard try goalManager.decrementProgress(goal) else { return }
			feedbackTrigger.toggle()
		} catch { saveFailure = .updateProgress }
	}
	...
	private func incrementProgress() {
		do {
			guard try goalManager.incrementProgress(goal) else { return }
			feedbackTrigger.toggle()
		} catch { saveFailure = .updateProgress }
	}
}
